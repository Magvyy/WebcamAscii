...
public class WebCamAsciiFlip {
            ...
            Thread thread = new Thread() {
                public void run() {
                    ...
                    while (arr != null) {
                        ...
                        index = y * (2 * cols) + 2 * (cols - x);
                        str.setCharAt(index, c);
                        index = y * (2 * cols) + 2 * (cols - x) + 1;
                        str.setCharAt(index, ' ');
                        arr = q.poll();
                    }
                    latch.countDown();
     ...
     * use the following command: "mvn exec:java -Dexec.mainClass="MyWebAscii.WebCamAsciiFlip"".
